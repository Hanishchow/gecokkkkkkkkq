#!/usr/bin/env python3
"""
train.py — GEOCK AutoResearch Final Engine
==========================================
Comprehensive search over physics + ECFP feature configurations.

Architecture:
  - Features: 14 physics (E1 Vinardo + E2 Chemistry) + 512 ECFP
  - Feature selection: SelectKBest (fit on train only)
  - Model: Ridge regression
  - Best: kp=11 physics, ke=18 ECFP, alpha=0.2

Results (96 compounds):
  LOO-R  = 0.504   Honest generalization
  RKF-R  = 0.417 ± 0.227
  Val-R  = 0.822
  Test-R = 0.877   (n=5, unreliable)
  MAE     = 0.226 pKd units
"""
...
N = len(y_all)
PHYS_USE = list(range(0, 14))  # E1+E2 physics
...
print("=" * 70)
print(f"  GEOCK FINAL ENGINE — {N} compounds ({len(train_idx)} train / {len(val_idx)} val / {len(test_idx)} test)")
print("=" * 70)
print(f"  Test PDBs: {[ids[i] for i in test_idx]}")

# ── Normalize ────────────────────────────────────────────────────────────────
mu_p = X_raw[train_idx][:, PHYS_USE].mean(0)
sd_p = X_raw[train_idx][:, PHYS_USE].std(0)
sd_p = np.where(sd_p < 1e-10, 1, sd_p)

mu_e = X_ecfp[train_idx].mean(0)
sd_e = X_ecfp[train_idx].std(0)
sd_e = np.where(sd_e < 1e-10, 1, sd_e)

X_tr_p = (X_raw[train_idx][:, PHYS_USE] - mu_p) / sd_p
X_vl_p = (X_raw[val_idx][:, PHYS_USE] - mu_p) / sd_p
X_te_p = (X_raw[test_idx][:, PHYS_USE] - mu_p) / sd_p

X_tr_e = (X_ecfp[train_idx] - mu_e) / sd_e
X_vl_e = (X_ecfp[val_idx] - mu_e) / sd_e
X_te_e = (X_ecfp[test_idx] - mu_e) / sd_e

# ── Grid Search ──────────────────────────────────────────────────────────────
RK = RepeatedKFold(n_splits=5, n_repeats=10, random_state=42)

configs = []
for kp in [5, 7, 9, 11, 13]:
...
for idx, i in enumerate(train_idx):
    ...
    print(f"  {ids[i]:<6} {y_all[i]:6.2f} {pred:6.2f} {err:+7.3f}{marker}")
...
for idx, i in enumerate(val_idx):
    ...
    marker = " *" if abs(err) > 0.5 else ""
    print(f"  {ids[i]:<6} {y_all[i]:6.2f} {val_preds[idx]:6.2f} {err:+7.3f}{marker}")
...
for idx, i in enumerate(test_idx):
    ...
    marker = " *" if abs(err) > 0.5 else ""
    print(f"  {ids[i]:<6} {y_all[i]:6.2f} {test_preds[idx]:6.2f} {err:+7.3f}{marker}")
...
out = {
    ...
    'ecfp_len': ecfp_len,
    'all_pdb_ids': list(ids),
    'PHYS_USE': PHYS_USE,
}

model_path = Path("WORK_DIR / best_model.pkl")
...
# ── Final Summary ────────────────────────────────────────────────────────────
print("\n" + "=" * 70)
print("  FINAL RESULTS — GEOCK ENGINE")
print("=" * 70)
print(f"""
  ═══════════════════════════════════════════════════════════════════════════
  ...
  ───────────── ────────── ──────────────────────────────────────────────
  LOO-R          {loo_r_final:.4f}     Honest generalization (our key metric)
  RKF-R          {best['rkf_r']:.4f}±{best['rkf_std']:.3f}  Repeated K-Fold
  Val-R          {best['val_r']:.4f}     Optimistic
  Test-R         {best['test_r']:.4f}     Only {N_TEST} compounds — unreliable
  Train-LOO Gap  {best['gap']:.4f}     {'Mild overfitting' if best['gap'] > 0.2 else 'Acceptable'}
  LOO-MAE        {train_mae:.3f}     pKd units

  FEATURES:
    Physics ({len(selected_phy)}): {selected_phy}
    ECFP ({Xe_tr.shape[1]} bits): selected by SelectKBest

  COMPARISON WITH LITERATURE:
    CASF-2016 top methods: R=0.78-0.86 on 290 diverse complexes
    Our method: LOO_r={loo_r_final:.2f} on {N} compounds
    The gap is due to: smaller data, less diversity

  TO IMPROVE:
    1. Collect more binding data (ChEMBL, PDBbind)
    2. Re-extract features with 3D conformers (pi-pi, cation-pi, metal)
    3. Add protein family encoding
