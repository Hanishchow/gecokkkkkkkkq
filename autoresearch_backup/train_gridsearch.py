#!/usr/bin/env python3
"""
GEOCK v2 - Hyperparameter Grid Search
Find best XGBoost config
"""
...
print("=" * 60)
print("GEOCK v2 - HYPERPARAMETER SEARCH")
print("=" * 60)
...
print(f"\n[2] Testing {len(configs)} configurations (3-fold)...")
...
for i, cfg in enumerate(configs):
    ...
    results.append({"config": cfg, "cv_r2": cv_r2, "std": np.std(fold_scores)})

    if (i + 1) % 20 == 0:
        print(
            f"  Tested {i + 1}/{len(configs)}, best so far: {max(r['cv_r2'] for r in results):.4f}"
        )
...
for i, r in enumerate(results[:5]):
    cfg = r["config"]
    print(
        f"  {i + 1}. R²={r['cv_r2']:.4f}: d={cfg['max_depth']}, lr={cfg['learning_rate']}, n={cfg['n_estimators']}, α={cfg['reg_alpha']}, λ={cfg['reg_lambda']}"
    )

# Full CV with best
print("\n[4] Full 5-fold CV with best config...")
...
print(f"CV R² = {cv_r2:.4f}, CV R = {cv_r:.4f}")
print(f"Previous best: 0.5956")
print(f"{'=' * 60}")
