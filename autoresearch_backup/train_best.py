#!/usr/bin/env python3
"""
Reproduce best result (CV R = 0.736) using physics + interaction features with proper setup.
"""
import pickle
import numpy as np
...
for i, c in enumerate(compounds):
    ...
    # ECFP (512) + mol (8) + physics (20) + interaction (20) = 560
...
r_vl = pearsonr(y_vl, ens_vl)[0]
r_te = pearsonr(y_te, ens_te)[0]
mae = np.mean(np.abs(y_te - ens_te))

print(f"Ensemble: Val R={r_vl:.4f}, Test R={r_te:.4f}, MAE={mae:.3f}")
...
cv_mean, cv_std = np.mean(cv_rs), np.std(cv_rs)
...
# ==================== SAVE ====================
output = {'models': models, 'weights': weights, 'configs': configs, 'mu': mu, 'sd': sd,
          'cv_r': cv_mean, 'cv_std': cv_std, 'val_r': r_vl, 'test_r': r_te, 'mae': mae}

with open('WORK_DIR / geock_best_reproduce.pkl', 'wb') as f:
    pickle.dump(output, f)

print(f"\n✓ Saved - CV R: {cv_mean:.4f}, Val: {r_vl:.4f}, Test: {r_te:.4f}")