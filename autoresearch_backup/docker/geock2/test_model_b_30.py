"""MODEL B: ElasticNet + ChemBERTa on 30 compounds"""
import numpy as np
...
os.environ['HF_TOKEN'] = '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'
...
def get_embedding(smiles):
    if not smiles:
        return np.zeros(768)
    try:
        inputs = tokenizer(smiles, return_tensors='pt', max_length=64, truncation=True)
        with torch.no_grad():
...
for i, c in enumerate(compounds):
    ...
    try:
        ...
        X_emb.append(get_embedding(c.get('smiles', '')))
        y.append(c['experimental_affinity'])
    except Exception as e:
        pass
    
    if (i+1) % 5 == 0:
        print(f"  Processed {i+1}/30 in {time.time()-t0:.1f}s...")
...
print(f"\n{'='*60}")
print(f"MODEL B (30 compounds): Pearson R = {r_b:.4f}, MAE = {mae_b:.4f}")
print(f"{'='*60}")
