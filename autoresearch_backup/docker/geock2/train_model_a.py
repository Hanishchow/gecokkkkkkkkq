"""MODEL A: ElasticNet + HETATM physics + ECFP4 on ALL 100 compounds"""
import numpy as np
...
for c in compounds:
    ...
    try:
        ...
        failed.append(c['pdb_id'])

X_phys, X_ecfp, y = np.array(X_phys), np.array(X_ecfp), np.array(y)
print(f"MODEL A: Loaded {len(X_phys)}/100 compounds in {time.time()-t0:.1f}s")
...
print(f"\n{'='*60}")
print(f"MODEL A RESULTS (ElasticNet + HETATM + ECFP4)")
print(f"{'='*60}")
print(f"n_features: {X_hybrid.shape[1]}")
print(f"Pearson R: {r_a:.4f}")
print(f"MAE: {mae_a:.4f} kcal/mol")
print(f"{'='*60}")

# Save model A
model_a_data = {'model_type': 'ElasticNet', 'alpha': best_alpha, 'l1_ratio': best_l1, 
                'scaler': scaler, 'n_phys': 60, 'n_ecfp': 512}
with open('/mnt/c/Users/yakka/Downloads/final/geock/model_a.pkl', 'wb') as f:
    pickle.dump(model_a_data, f)
print("Saved model_a.pkl")
