"""
simple_affinity_hetatm.py - Ridge with Fixed HETATM Parsing
Uses parse_pocket_and_ligand() from patch_parse.py
"""
...
def compute_features(ligand_coords, ligand_types, pocket_coords, pocket_types, center=None):
    """Compute 60 physics-based features."""
...
def load_data():
    compounds_file = '/mnt/c/Users/yakka/Downloads/geock_110_data/compounds.json'
    ...
    for c in compounds:
        pdb_file = os.path.join(data_dir, c['pdb_id'], f"{c['pdb_id']}_pocket.pdb")
        if not os.path.exists(pdb_file):
            failed.append((c['pdb_id'], 'missing'))
            continue
        try:
            rec_coords, rec_types, lig_coords, lig_types, _, _ = \
                parse_pocket_and_ligand(pdb_file, cutoff=10.0)
            if len(rec_coords) < 10 or len(lig_coords) < 3:
                failed.append((c['pdb_id'], 'too_few_atoms'))
                continue
            features = compute_features(lig_coords, lig_types, rec_coords, rec_types)
            X.append(features)
            y.append(c['experimental_affinity'])
        except Exception as e:
            failed.append((c['pdb_id'], str(e)[:30]))
    
    print(f"Loaded {len(X)} compounds, failed: {len(failed)}")
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("Ridge Regression with Fixed HETATM Parsing")
    print("=" * 60)
    
    X, y, failed = load_data()
