"""Ridge with Fixed HETATM - Original features exactly"""
import numpy as np
...
def compute_features(ligand_coords, ligand_types, pocket_coords, pocket_types, center):
    """Original 60 features - EXACT copy from simple_affinity.py"""
    ...
    # EXACT same distance calculations as original
    all_dists = np.array([np.linalg.norm(lc - pc) 
                          for lc in ligand_coords for pc in pocket_coords])
    lig_dists = np.array([min(np.linalg.norm(lc - pc) for pc in pocket_coords) 
                          for lc in ligand_coords])
    rec_dists = np.array([min(np.linalg.norm(lc - pc) for lc in ligand_coords) 
                          for pc in pocket_coords])
    ...
    # Pocket composition from TYPES (not pocket_features array)
    ...
    # Interaction scoring - original nested loops
    contact_score = 0.0
    hydro_score = 0.0
    hbond_score = 0.0
